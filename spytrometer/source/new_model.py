...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, mz_values):
        ...
        # Calculate distance matrix for m/z values
        ...
        pe = torch.cat((pe_sin, pe_cos), dim=1)  # shape: (150, d_model)

        match_encoding = torch.zeros(150, self.d_model, 1).to(self.device)  # Initialize with zeros

        for i in range(150):  # Loop over the 150 peaks
            for k in range(28):  # Loop over the 28 amino acids
                a_ij = match_matrices[k, :, i]  # Extract mass diff matrix for this amino acid, shape: 150
                p_i = pe[i, :]  # Extract the i-th element of the positional encoding, shape: (64, 1)
                a_ij_exp = a_ij.unsqueeze(1)  # shape: (150, 1)
                a_ij_exp = a_ij_exp.unsqueeze(2) #shape (150, 1, 1)
                #p_i_exp = p_i.unsqueeze(2)  # shape: (64, 1, 1)
                p_plus_pe = p_i + pe  # shape: (150, 64, 1)
                # Broadcasting p_plus_pe and multiplying with a_ij_exp
                add = a_ij_exp * p_plus_pe # shape: (150, 64, 1)
                match_encoding += add  # Accumulate the match encodings

        match_encoding = match_encoding.squeeze(0)  # Remove the first dimension (batch size)

        # Reshape to (150, 64)
        match_encoding = match_encoding.view(-1, self.d_model)
        print("MATCH ENC", match_encoding.shape)
