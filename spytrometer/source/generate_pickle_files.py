            ...
            for spec in spectra:
                ...
                if scannum is not None and scannum in scans:
                    ...
                    if not row.empty:
                        ...
                        #print(mz_values)
                        #print(mz_values.dtype())
                        intensities = spec.get_peaks()[1]
                        #added 
                        mz_to_intensity = defaultdict(float)

                        for mz, intensity in zip(mz_values, intensities):
                            rounded_mz = round(mz, 2)
                            if intensity > mz_to_intensity[rounded_mz]:
                                mz_to_intensity[rounded_mz] = intensity
                        merged_mz_values = np.array(list(mz_to_intensity.keys()))
                        merged_intensities = np.array(list(mz_to_intensity.values()))
                        # old stuff

                        max_intensity = spec.getMaxIntensity()
                        #mask = intensities >= 0.01 * max_intensity
                        mask = merged_intensities >= 0.01 * max_intensity #new
                        #filtered_intensities = intensities[mask] 
                        #filtered_mz_values = mz_values[mask]
                        filtered_intensities = merged_intensities[mask]  #new
                        filtered_mz_values = merged_mz_values[mask] #new
                        ...
                        mz_values_masked = filtered_mz_values[mask_2]
                        # Sort by intensity in descending order
                        sorted_indices = intensities_scaled_masked.argsort()[::-1]

                        # Keep only the 150 largest intensities and corresponding mz values
                        top_150_indices = sorted_indices[:150]
                        top_150_intensities = intensities_scaled_masked[top_150_indices]
                        top_150_mz_values = mz_values_masked[top_150_indices]

                        results.append({
                            'file': mzml_file,
                            'scan': scannum,
                            'mz_values': list(top_150_mz_values),
                            'intensity': list(top_150_intensities),
